...
## 你的任务
分析用户的自然语言需求，推荐 3 个最匹配的图表方案。必须返回 3 个不同的图表类型。
...
## 输出格式（非常重要）
必须返回 JSON 对象，包含 recommendations 数组字段：
{
  "recommendations": [
    {
      "type": "图表类型",
      ...
      "reason": ["推荐理由1", "推荐理由2"],
      "confidence": 0.95,
      "preview": { "dataset": "sales|users|conversion", "description": "数据说明" }
    },
    {
      "type": "图表类型2",
      "title": "图表标题2",
...
关键要求：
1. 必须包含 recommendations 字段
2. recommendations 必须是数组，包含 3 个推荐
3. 3个推荐必须使用不同的图表类型（如 line, bar, pie）
4. confidence 应该递减（第一个最高）
5. 返回必须是 JSON 对象格式，不是数组
...
示例输出：
{
  "recommendations": [
    ...
    },
    {
      "type": "bar",
      "title": "各日期销售额对比",
      "reason": ["柱状图适合对比不同时段的数值", "排名展示清晰直观"],
      "confidence": 0.85,
      "preview": { "dataset": "sales", "description": "按日期汇总销售额并对比" }
    },
    {
      "type": "heatmap",
      "title": "销售时段热力分布",
      "reason": ["热力图展示时间和数值的密集程度", "适合发现高峰时段规律"],
      "confidence": 0.75,
      "preview": { "dataset": "sales", "description": "按时段统计销售热度分布" }
    }
  ]
}`;
